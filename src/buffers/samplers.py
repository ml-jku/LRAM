...
class DomainWeightedRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        indices_per_domain = self.make_indices_per_domain()
        ...
        for i in range(len(self)):
            ...
            if domain_to_sample_idx[domain_idx] == self.num_samples_per_domain:
                ...
                sample_idx = domain_to_sample_idx[domain_idx]
                # regenerate indices
                indices_per_domain[domain_idx] = self.make_indices_single_domain(domain_idx=domain_idx)
    ...
    def make_indices_per_domain(self): 
        indices_per_domain = {}
        for domain, w in self.weights.items():
            indices_per_domain[domain] = self.make_indices_single_domain(domain=domain, weights=w)
        return indices_per_domain

    def make_indices_single_domain(self, domain_idx=None, domain=None, weights=None):
        if domain is None: 
            domain = list(self.weights.keys())[domain_idx]
        if weights is None:
            weights = self.weights[domain]
        indices = (torch.multinomial(
            weights, self.num_samples_per_domain, self.replacement, generator=self.generator
        ) + self.domain_to_start_idx[domain]).to(torch.int32)
        return indices
...
class DatasetFromSampler(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.sampler_list is None:
            ...
            self.sampler_list = torch.empty(len(self.sampler), dtype=torch.int32)
            for i, idx in enumerate(self.sampler):
                self.sampler_list[i] = idx
